...
class Canvas extends JComponent {
    ...
    public void paintComponent(Graphics g) {
        ...
        model.getEdges().forEach(e -> {
            ...
            g2d.draw(new Line2D.Double(scaledStart.getLong(), scaledStart.getLat(), scaledEnd.getLong(), scaledEnd.getLat()));
        });


        g2d.setColor(Color.RED);
        g2d.setStroke(new BasicStroke(2));
        shortestPath.stream().skip(1).forEach(v -> {
            Vertex scaledStart = shortestPath.get(shortestPath.indexOf(v) - 1).scale(scale, model.getLatMin(), model.getLongMin());
